 ...
 *        classification task, passing real-valued predictions through a
 *        softmax to get a probability distribution over classes.
 *        + supports for negative date
 *
 * This layer should be preferred over separate
 * SoftmaxLayer + MultinomialLogisticLossLayer + negative support
 * as its gradient computation is more numerically stable.
 * At test time, this layer can be replaced simply by a SoftmaxLayer.
 *
 * @param bottom input Blob vector (length 2)
 ...
 */
template <typename Dtype>
class SoftmaxWithLossLayerNegative : public LossLayer<Dtype> {
 public:
  explicit SoftmaxWithLossLayerNegative(const LayerParameter& param)
      : LossLayer<Dtype>(param),
        softmax_layer_(new SoftmaxLayer<Dtype>(param)) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_SOFTMAX_LOSS_NEGATIVE;
  }
  virtual inline int ExactNumBottomBlobs() const { return -1; }
  virtual inline int MinBottomBlobs() const { return 2; }
  virtual inline int MaxBottomBlobs() const { return 3; }
