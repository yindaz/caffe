...
//	}
  ...
  for (int i = 0; i < num; ++i) {
    for (int j = 0; j < spatial_dim; j++) {
			...
			if (isneg) // negative label 0.1
			{
				loss -= log(std::max( 1 - prob_data[i * dim + ori * spatial_dim + j],
									 Dtype(FLT_MIN)));
			}
			else
			{
...
//        Dtype r = label[i * spatial_dim + j] - l;
        if (ExternData.MiniBatchIsneg[i]) // negative label 0.1
        {
        	Dtype pk = bottom_diff[i * dim + l * spatial_dim + j];
					Dtype wi = -pk / ( (Dtype)1. - pk ); 
	 			for (int k = 0; k < dim; ++k)
          {
            if ( k != l )
            {
              bottom_diff[i * dim + k * spatial_dim + j] *= wi;
            }
          }
        }
        else
