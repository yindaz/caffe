...
void DataProcess::SetupLMDB( const MDB_dbi &dbi, const MDB_txn &txn)
{
	if (!ACTIVATE) return;
...
void DataProcess::SetupDataNumber(const MDB_env* &env_)
{
	if (!ACTIVATE) return;
...
void DataProcess::FindOutlier( Dtype* top_diff, Dtype* bottom_data, int M, int N, int K)
{
	if (!ACTIVATE) return;
